...
namespace Abetare
{
	...
	sprintf(cmd_neighs, triangleNeighbors ? "n" : "");
	sprintf(cmd_D, useConformingDelaunay ? "D" : "");
	
	if(angleConstraint > 0)
	    sprintf(cmd_angle, "q%f", angleConstraint);
	else
	    sprintf(cmd_angle, "");
	if(areaConstraint > 0)
	    sprintf(cmd_area, "a%f", areaConstraint);
	else
	    sprintf(cmd_area, "");
	
	sprintf(cmd, "-pz%s%s%s%s -Q", cmd_neighs, cmd_D, cmd_angle, cmd_area);
	
	

	ShewchukTriangle::triangulateio tin;
	ShewchukTriangle::triangulateio tout;
	int scount = 0;
	int vcount = 0;
	
	
	tin.numberofpoints  = totalNrVertices;
	tin.pointlist       = (REAL *) malloc(2 * totalNrVertices * sizeof(REAL));
	memcpy(tin.pointlist, verticesPolyAndHoles, 2 * totalNrVertices * sizeof(REAL));
	
	
	tin.numberofsegments = totalNrVertices;
	tin.segmentlist      = (int *) malloc(2 * tin.numberofsegments * sizeof(int));
	for(int i = 0; i <= nrHoles; ++i)
	{
	    for(int j = 0; j < nrVerticesPerContour[i]; ++j, scount = scount + 2)
	    {
		tin.segmentlist[scount]     = vcount + j;
		tin.segmentlist[scount + 1] = vcount + (j + 1) % nrVerticesPerContour[i];	    
	    }	
	    vcount += nrVerticesPerContour[i];	
	}
	
	if(nrHoles > 0)
	{
	    tin.numberofholes = nrHoles;
	    tin.holelist      = (REAL *) malloc(2 * nrHoles * sizeof(REAL));
	    memcpy(tin.holelist, ptsInsideHoles, 2 * nrHoles * sizeof(REAL));
	}
	
	triangulate(cmd, &tin, &tout, NULL);
	
	if(triangleVertices)
	{
	    triangleVertices->resize(tout.numberofpoints * 2);
	    memcpy(&((*triangleVertices)[0]), tout.pointlist, 2 * tout.numberofpoints * sizeof(double));
	}
	
	if(triangleIndices)
	{
	    triangleIndices->resize(tout.numberoftriangles * tout.numberofcorners);
	    memcpy(&((*triangleIndices)[0]), tout.trianglelist, tout.numberofcorners * tout.numberoftriangles * sizeof(int));
	}
	
	if(triangleNeighbors)
	{
	    triangleNeighbors->resize(tout.numberoftriangles * 3);
	    memcpy(&((*triangleNeighbors)[0]), tout.neighborlist, 3 * tout.numberoftriangles * sizeof(int));
	}
	
	
	if(tout.pointlist && (tout.pointlist != tin.pointlist))
	    free(tout.pointlist);
	if(tout.pointattributelist && (tout.pointattributelist != tin.pointattributelist))
	    free(tout.pointattributelist);
	if(tout.pointmarkerlist && (tout.pointmarkerlist != tin.pointmarkerlist))
	    free(tout.pointmarkerlist);
	
	
	if(tout.segmentlist && (tout.segmentlist != tin.segmentlist))
	    free(tout.segmentlist);
	if(tout.segmentmarkerlist && (tout.segmentmarkerlist != tin.segmentmarkerlist))
	    free(tout.segmentmarkerlist);
	
	if(tout.holelist && (tout.holelist != tin.holelist))
	    free(tout.holelist);
	
	if(tout.regionlist && (tout.regionlist != tin.regionlist))
	    free(tout.regionlist);
	
	if(tout.edgelist && (tout.edgelist != tin.edgelist))
	    free(tout.edgelist);
	if(tout.edgemarkerlist && (tout.edgemarkerlist != tin.edgemarkerlist))
	    free(tout.edgemarkerlist);
	if(tout.normlist && (tout.normlist != tin.normlist))
	    free(tout.normlist);
	
	if(tout.trianglelist && (tout.trianglelist != tin.trianglelist))
	    free(tout.trianglelist);
	if(tout.triangleattributelist && (tout.triangleattributelist != tin.triangleattributelist))
	    free(tout.triangleattributelist);
	if(tout.trianglearealist && (tout.trianglearealist != tin.trianglearealist))
	    free(tout.trianglearealist);
	if(tout.neighborlist && (tout.neighborlist != tin.neighborlist))
